        ...
        $('#users-table').DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row) {
                    ...
                    return `
    <select class="form-select form-select-sm btn-action" onchange="handleAction(this.value, '${row.id}', '${permissionshow}', '${permissiondelete}')" aria-label="Actions" style="width: auto;">
        <option value="" class="text-center" style="color: gray;" selected disabled>الخيارات</option>
        <option value="show" class="text-center" data-url="${permissionshow}" style="color: #274373;">عرض</option>
        <option value="edit" class="text-center" data-url="${permissiondelete}" style="color: #eb9526;">حذف</option>
    </select>
`;
    ...
    function handleAction(action, id, showUrl, deleteUrl) {
        if (action === 'show') {
            window.location.href = showUrl; // Redirect to the "show" page
        } else if (action === 'edit') {
            window.location.href = deleteUrl; // Redirect to the "edit" (delete) page
        } else {
